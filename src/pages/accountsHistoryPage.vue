  ...
  <div class="page-content">
    <div class="row">
      ...
      <div class="col-md-10">
        ...
        <KaccountsHistoryPage :list="list"></KaccountsHistoryPage>
      </div>
    </div>
  </div>
  <KFooter></KFooter>
...
import KaccountsHistoryPage from '../component/accountsHistoryPage.vue'
...
export default {
  components: {
    KHeader,KSidebar,KFooter,KaccountsHistoryPage
  },
  name: 'tableTest',
  data () {
    return {
  ...
  methods: {
    getData: function(){
      ...
      apiService.get('/api/user/info/'+condition+'/overall',{
        ...
        apiService.get('/v1/order/user/'+vm.userId,{  
        }).then(function(response){
          console.log(response); 
          if (response.data.code == "00000000") {

            vm.list = dataAadapter.mainPageOrdersToItem(response.data.data.orders);
          }
        }).then(function(){
          $( document ).ready(function() {
            $('#infoUpdate').dataTable( {
                // paging: false,
                searching:false,
                "aoColumnDefs": [ { 'bSortable': false, 'aTargets': [ 0,1,2,3,4,5,7,8,9 ] }],
                "order": [[ 6, "desc" ]]
            });
          });
        });
