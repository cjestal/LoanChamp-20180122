...
<div id="tableTest">
  ...
  <div class="page-content">
    <div class="row">
      ...
      <div class="col-md-10">
        <KTable :list="list" tableType="4"></KTable>
      </div>
    </div>
  </div>
  <KFooter></KFooter>
...
export default {
  ...
  methods: {
    getData: function(){
      ...
      var vm = this;
      var uwId = localStorage.getItem(constantService.localStorage.userId);
      apiService.get('/v1/order/list/underwriter/'+uwId,{
          pageNum: 1,
          pageSize: 999
        ...
        for (var i = response.data.data.orders.length - 1; i >= 0; i--) {
          tempList.push(dataAadapter.orderUnderwritingToItem(response.data.data.orders[i]));
        }
        vm.list = tempList;
      }).then(function(){
         $('#example').dataTable( {
              "aoColumnDefs": [
                  { 'bSortable': false, 'aTargets': [ 0,1,4,5,9,10,11,12 ] }
               ],
      ...
      //       "scrollX": true,
      //       "aoColumnDefs": [
      //           { 'bSortable': false, 'aTargets': [ 0,1,4,5,9,10,11 ] }
      //        ],
      //        "lengthMenu": [[10, 30, 50], [10, 30, 50]]
      //   });
